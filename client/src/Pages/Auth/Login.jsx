...
const Login = () => {
  ...
  const handleLogin = async(e) => {
    e.preventDefault();
    try {
        const  res = await axios.post("http://localhost:5002/api/v1/auth/login", { email, password,})
  ...
  return (
    <div className="w-full h-full  bg-gradient-to-r from-blue-300 to-blue-600 flex justify-center ">
      <div className="flex  px-20 mt-24 mb-6 w-[980px] items-center gap-x-6 justify-between rounded-xl h-screen  bg-white shadow-xl  max-[768px]:mx-2 ">
        <form className="input-container mb-12 w-1/2 h-1/2  flex flex-col gap-y-3 max-[768px]:w-full" onSubmit={handleLogin}>
          <span>Login</span>
          <div>
            <IoMdMail size={16} />
            <input
